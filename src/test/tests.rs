...
mod tests {
    ...
    fn test_mock_db_with_token_cipher() {
        ...
        db.save_hackatime_connection(user_id, "acc_456".into(), encrypted_token.clone());

        let conns = db.hackatime_connections.lock().unwrap();
        let conn = conns.get(&user_id).unwrap();
